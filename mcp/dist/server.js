...
function generateColorScale(seedHex) {
    const { h, c } = hexToHct(seedHex);
    const scale = {};
    for (const [key, tone] of Object.entries(TONE_SCALE)) {
        scale[key] = hctToHex(h, c, tone);
    }
    return scale;
}
function generateLightTheme(seedHex) {
    const { h, c } = hexToHct(seedHex);
    return {
...
function generateDarkTheme(seedHex) {
    const { h, c } = hexToHct(seedHex);
    return {
...
function generateDesignSystem(primaryHex) {
    return {
        primary: primaryHex,
...
function exportToTailwind(system) {
    const config = {
        theme: {
            extend: {
                colors: {
...
function exportToJson(system) {
    const tokens = {
        $schema: "https://design-tokens.github.io/community-group/format/token.json",
        color: {
            primary: {},
            semantic: {
                light: {},
...
function kMeansClustering(pixels, k = 6, maxIterations = 20) {
    if (pixels.length === 0)
        return [];
...
async function extractColorsFromImageUrl(imageUrl, colorCount = 6) {
    // Fetch image
...
function selectPrimaryColor(colors) {
    if (colors.length === 0)
        return null;
...
server.tool("generate-design-system", "Generate a complete Material Design 3 design system from a primary color", {
    primaryColor: z
        .string()
        .regex(/^#[0-9A-Fa-f]{6}$/)
...
// Tool: export-tokens
server.tool("export-tokens", "Export design system tokens in various formats (css, tailwind, json, figma)", {
    primaryColor: z
        .string()
        .regex(/^#[0-9A-Fa-f]{6}$/)
...
// Tool: analyze-color
server.tool("analyze-color", "Analyze a color and get its HCT values and accessibility info", {
    color: z
        .string()
        .regex(/^#[0-9A-Fa-f]{6}$/)
    ...
    const analysis = {
        ...
        accessibility: {
            textOnWhite: contrastWithWhite >= 4.5
                ? "AA"
                : contrastWithWhite >= 3
                    ? "AA-Large"
                    : "Fail",
            textOnBlack: contrastWithBlack >= 4.5
                ? "AA"
                : contrastWithBlack >= 3
                    ? "AA-Large"
                    : "Fail",
...
server.tool("suggest-palette", "Suggest complementary, analogous, or triadic colors based on a primary color", {
    primaryColor: z
        .string()
        .regex(/^#[0-9A-Fa-f]{6}$/)
        .describe("Primary color in hex format"),
...
server.tool("extract-colors-from-image", "Extract dominant colors from an image URL using K-Means clustering", {
    imageUrl: z
        .string()
        .url()
...
server.tool("create-design-system-from-image", "Extract colors from an image and generate a complete Material Design 3 design system", {
    imageUrl: z
        .string()
        .url()
        .describe("URL of the image to extract colors from"),
